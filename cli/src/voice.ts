...
async function transcribeWithParakeet({
  ...
  try {
    // Convert to OGG if needed (Parakeet accepts various formats)
...
export async function transcribeWithVLLM({
  ...
  try {
    // vLLM expects OpenAI-compatible audio transcription endpoint
    const finalBuffer = audioBuffer
...
export async function transcribeAudio({
  ...
  // Resolve provider priority: explicit param > env var > api key inference
